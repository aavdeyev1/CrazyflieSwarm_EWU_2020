...
def run_sequence(scf, params):
    ...
    poshold(cf, 2, base)   # fly up a tiny bit (base), hold for 2 secs

    ramp = fs * 2
    for r in range(ramp):   # fly up in increments of ramp
        cf.commander.send_hover_setpoint(0, 0, 0, base + r * (z - base) / ramp)
        time.sleep(fsi)

    poshold(cf, 2, z)   # hover at the position in params, hold for 2 secs

    for _ in range(2):   # circle around at a certain height
    ...
    for r in range(ramp):   #fly down in increments of ramp
    ...
    cf.commander.send_position_setpoint(0, 0, 1.5, 0)
